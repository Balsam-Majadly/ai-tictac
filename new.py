...
def check_winner():
    ...
    for combination in winning_combinations:
        if all(['text'] == 'X' for button in combination):
...
# Modify the b_click, c_click, d_click functions to handle the AI's move
def b_click(b):
...
my_s1 = simpledialog.askstring("Input", "Enter first player name", parent=root)
my_s2 = simpledialog.askstring("Input", "Enter second player name", parent=root)
while my_s1 == my_s2:
    messagebox.showerror('3D-tic-tac-toe', 'Enter different names')
    my_s1 = simpledialog.askstring("Input", "Enter first player name", parent=root)
    my_s2 = simpledialog.askstring("Input", "Enter second player name", parent=root)
